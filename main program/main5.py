### LANDING LUMAYAN PRESISI
### MASIH KURANG MUNDUR
### TOLERANSI X DITAMBAH 2 UNTUK 200M (Yang ini udah diganti, harusnya lebih perfek)
...
#If Aruco jauh dan hanya di 1 sisi
        if id == 5 and z <= 400 and z >= 200 :

            print("x : ", x)
            print("y : ", y)

            if x <= 40 and x >= 16 and y <= 20 and y >= 7:
                print("ID = ", id, "Terbaca, bergerak mundur")
                velocity(-0.1,0, 0, 1)
            elif x >= -40 and x <= 0 and y <= 20 and y >= 7:
                print("ID = ", id, "Terbaca, bergerak maju")
                velocity(0.1, 0, 0, 1)
            elif x <= 16 and x >= 2 and y >= -40 and y <= 3:
                print("ID = ", id, "Terbaca, bergerak kanan")
                velocity(0, 0.1, 0, 1)
            elif x <= 16 and x >= 2 and y <= 40 and y >= 20:
                print("ID = ", id, "Terbaca, bergerak kiri")
                velocity(0, -0.1, 0, 1)

# IF (x,y) ArUco masuk toleransi, tapi masih diatas 100 cm
            elif x <= 16 and x >= 2 and y <= 20  and y >= 3:
                print("Masuk toleransi, bergerak turun")
                velocity(0, 0, 0.2, 1)

        if id == 5 and z <= 200 and z >= 150 :

            print("x : ", x)
            print("y : ", y)

            if x <= 20 and x >= 8 and y <= 10 and y >= 4:
                print("ID = ", id, "Terbaca, bergerak mundur")
                velocity(-0.1, 0, 0, 1)

            elif x >= -20 and x <= 2 and y <= 10 and y >= 4:
                print("ID = ", id, "Terbaca, bergerak maju")
                velocity(0.1, 0, 0, 1)

            elif x <= 9 and x >= 6 and y >= -20 and y <= 5:
                print("ID = ", id, "Terbaca, bergerak kanan")
                velocity(0, 0.1, 0, 1)

            elif x <= 9 and x >= 6 and y <= 20 and y >= 10:
                print("ID = ", id, "Terbaca, bergerak kiri")
                velocity(0, -0.1, 0, 1)

# IF (x,y) ArUco masuk toleransi, tapi masih diatas 100 cm
            elif x <= 9 and x >= 6 and y <= 10 and y >= 4:
                print("Masuk toleransi, bergerak turun")
                velocity(0, 0, 0.2, 1)

#######################################  LANDING  ###################################################

        if id == 5 and x <= 8 and x >= 6 and y <= 9 and y >= 5 and z <=150 and z>=50:
